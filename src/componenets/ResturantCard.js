...
export default function ResturantCard({item}) {
 ...
 {item.data.name}
   ...
   >
     {" "}
     {item.data.cuisines.slice(0, 4).join()}
   </span>

   <div className="text-gray-600 flex justify-between my-4 text-xs">
    {
      item.data.avgRating>4
      ?

      <p className="btn bg-green-600 text-white    text-xs p-1">
      <i className="fa fa-star"></i> {item.data.avgRating
      }
      </p>
      :
      item.data.avgRating>3?
    <p className="btn bg-yellow-600 text-white    text-xs p-1">
    <i className="fa fa-star"></i> {item.data.avgRating
    }
  </p>
  :
  <p className="btn bg-red-600 text-white    text-xs p-1">
  <i className="fa fa-star"></i> {item.data.avgRating
  }
</p>
     ...
     <p>{item.data.deliveryTime
     } mins</p>
     <p>{item.data.costForTwoString}</p>
   </div>
   <hr />
   <div className="flex items-center flex-wrap ">
     <span className="text-sm py-2 flex">
       <svg
