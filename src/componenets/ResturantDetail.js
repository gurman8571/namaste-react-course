...
console.log(reload);
  ...
  const fetchdetails = async () => {
    const { data } = await axios.get(
      `https://www.swiggy.com/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=28.6738274&lng=77.1642584&restaurantId=${id}`
    );
...
setmenu(data.data?.cards[2]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1].card.card.itemCards
  ...
  return (
    <div className="pt-20 ">
      <div className="bg-black    h-72 flex px-20">
        <img
          className="object-contain w-80 m-10 h-48  pl-16"
          src={`${img_url}/${resturant?.cloudinaryImageId}`}
        />
        ...
        <div className="m-14">
          ...
          <p className="text-sm text-gray-300 pt-4">
            {resturant?.cuisines.slice(0, 4).join()}
          </p>
          <p className="text-sm text-gray-300 ">
            {resturant?.city},{resturant?.locality},{resturant?.areaName}{" "}
          </p>
          <br />
          <div className="flex text-white mb-4">
            <div className="w-full">
              <p>
                <i className="fa fa-star "></i> {resturant?.avgRatingString} |
            ...
            <div className="w-full">
              <p >{resturant?.sla?.deliveryTime} mins  |</p>
              <p className="text-gray-300 text-xs"> Delivery time</p>
            </div>
            <div className="w-full">
              <p> {resturant?.costForTwo/100}  |</p>
        ...
        <div className="m-10">
          ...
          >
            <div className="py-4 flex text-white font-bold">
              ...
              {offers[0]?.info?.header} | Use code {offers[0]?.info?.couponCode}
            </div>
            <div className="flex text-white font-bold">
              ...
              {offers[1]?.info?.header} | Use code {offers[1]?.info?.couponCode}
            </div>
          </div>
        </div>
        <div></div>
      ...
      <section className="flex justify-center mt-8">
        ...
        <div className="mt-4  w-3/6">
          {menu?.map((item, index) => {
            return (
             <Fooditem item={item?.card?.info} key={item?.card?.info?.id}/>
            );
          })}
      
        </div>
        <div className="w-2/6 sticky mt-8">
        <Cart setreload={setreload} rest={resturant?.name} img={resturant?.cloudinaryImageId} />
        </div>
      </section>
    </div>
  );
